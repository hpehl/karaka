...
public class ServletServerCache implements ServletContextListener
{
    ...
    public void contextInitialized(ServletContextEvent servletContextEvent)
    {
        ...
        String maxSize = servletContext.getInitParameter("karaka.server.cache.maxsize");
        if (maxSize != null)
        {
            cache.setMaxSize(Integer.parseInt(maxSize));
        }

        String expiration = servletContext.getInitParameter("karaka.server.cache.expiration");
        if (expiration != null)
        {
